...
- Time series of GNN generalization

- - - Train/test config

    - - GNN train/test on |V|=100,  $$\sigma_{J}=0.3, \sigma_{b}=0.25$$

    - NodeGNN without skip-connection also showed band pattern.

    - - Band pattern does not occur only in skip-connection or Attention. 
      - Skip-connection or attention are just assistance helping NodeGNN to fit more on train dataset.

    - Generalization show repetitive pattern. (Wide → band → Wide → …) As epoch increase, blue region moves from lower degree to higher degree graph. When blue region change to wide pattern, it moves to higher degree graph.

    - - It is possible to proceed to the next step(move right) only after fitting on graphs of low degree. Band pattern means that GNN sufficiently fitted on graphs of specific degree group. If not, it shows wider generalization pattern.

- - - $$\mathcal{M}$$ = {NodeGNN+skip-connection, MsgGNN+skip-connection, Attention+skip-connection, Attention}
    - However, we found that the generalization of GNNs without attention-based aggregation was bounded by ~ ~ ~
    - What are those band pattern?

- - -  Add the figure of degree distribution

- - - Describe the effect of attention/skip-connection.

    - - Possibly gaussian-fit to the histogram of degrees from the blue graphs and compare the estimated variance.
...
2. - - Attention network show most strict band-like generalization pattern. Generalization performance of NodeGNN, however, outperforms the others over entire test graph.
...
3. - GNN training on |V|=16, $$\sigma_{J}$$ = 0.3, $$\sigma_{b}$$ = 0.25
...
2. - GNN training on |V|=16, $$\sigma_{J}$$ = 0.75/0.6, $$\sigma_{b}$$ = 0.25
...
1. 1. BatchNorm(Only in Output function)
